...
export class VacationDateFromEditComponent extends DefaultEditor implements OnInit {
    ...
    setValue(event: any) {
        ...
        const vacationTo = this.cell.getRow().cells[1].newValue;

        if (vacationTo) {
            const request = new VacationCalculateDaysRequestModel();
            request.from = Utils.getDateTimeToSend(this.cell.newValue);
            request.to = Utils.getDateTimeToSend(vacationTo);
