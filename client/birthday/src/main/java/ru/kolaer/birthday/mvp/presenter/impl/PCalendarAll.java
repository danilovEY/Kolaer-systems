	...
	public void notifySelectedDate(LocalDate date) {
		if (this.observerCalendar != null) {
			...
			if (this.editorKid != null) {
				...
				final EmployeeOtherOrganizationBase[] usersBirthdayAll = this.editorKid.getUSNetwork().getRestfulServer().getKolaerDataBase()
				...
				if(usersBirthdayAll.length != 0) {
					...
					for (EmployeeOtherOrganizationBase user : usersBirthdayAll) {
						obs.setValue(value);
						value += step;
						final UserModel userModel = new UserModelImpl(user);
						users.add(userModel);
