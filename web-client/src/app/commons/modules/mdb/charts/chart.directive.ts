...
export class MdbBaseChartDirective implements OnDestroy, OnChanges, OnInit, Colors {
...
function generateColor(index: number): number[] {
	return MdbBaseChartDirective.defaultColors[index] || getRandomColor();
}
...
function generateColors(count: number): Array<number[]> {
	...
	for (let i = 0; i < count; i++) {
		colorsArr[i] = MdbBaseChartDirective.defaultColors[i] || getRandomColor();
	}
	return colorsArr;
}
