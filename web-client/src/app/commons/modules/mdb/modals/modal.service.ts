...
export class MdbModalService {
	...
	// protected backdropRef: ComponentRef<MdbModalBackdropComponent>;
	protected backdropRef: ComponentRef<MdbModalBackdropComponent> | any;
	private _backdropLoader: ComponentLoader<MdbModalBackdropComponent>;
	private modalsCount = 0;
	// private lastDismissReason = '';
	private lastDismissReason: any = '';

	private loaders: ComponentLoader<MdbModalContainerComponent>[] = [];

	// public constructor(private clf: MdbComponentLoaderFactory) {
	public constructor(private clf: MdbComponentLoaderFactory, private el: ElementRef, private v: ViewContainerRef, private r: Renderer) {
		//   this._backdropLoader = this.clf.createLoader<MdbModalBackdropComponent>(null, null, null);
		this._backdropLoader = this.clf.createLoader<MdbModalBackdropComponent>(this.el, this.v, this.r);
		...
		if (this.modalsCount === 1) {
			...
			if (isBackdropEnabled && isBackdropInDOM) {
				this._backdropLoader
					.attach(MdbModalBackdropComponent)
					.to('body')
	...
	_showModal(content: any): MDBModalRef {
		...
		const modalContainerRef = modalLoader
			...
			.attach(MdbModalContainerComponent)
			.to('body')
	...
	private _createLoaders(): void {
		// const loader = this.clf.createLoader<MdbModalContainerComponent>(null, null, null);
		const loader = this.clf.createLoader<MdbModalContainerComponent>(this.el, this.v, this.r);
	...
	private removeLoaders(level: number): void {
		...
		this.loaders.forEach((loader: ComponentLoader<MdbModalContainerComponent>, i: number) => {
